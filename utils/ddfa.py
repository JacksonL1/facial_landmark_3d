...
import pickle
import argparse
from .wj_io import _numpy_to_tensor, _load_cpu, _load_gpu
from .params import *
...
def parse_param_240(param):
    ...
    p = p_[:, :3]
    offset = p_[:, -1].reshape(3, 1)
    alpha_shp = param[12:211].reshape(-1, 1)
    alpha_exp = param[211:].reshape(-1, 1)
    return p, offset, alpha_shp, alpha_exp


def reconstruct_vertex(param, shape, whitening=True, dense=False, transform=True):
